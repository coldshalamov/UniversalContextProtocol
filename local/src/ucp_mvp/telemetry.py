...
and learning feedback. This is foundation for online learning
...
class SQLiteTelemetryStore(TelemetryStore):
    ...
    def log_routing_event(self, event: RoutingEvent) -> None:
        ...
        self._db.execute(
            ...
            (event_id, session_id, request_id, trace_id, timestamp, query_hash, query_text,
    ...
    def log_tool_call(self, event: ToolCallEvent) -> None:
        ...
        self._db.execute(
            ...
            (event_id, session_id, request_id, routing_event_id, trace_id, timestamp, tool_name,
             success, error_class, error_message, execution_time_ms, was_selected, selection_rank)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                str(event.event_id),
    ...
    def get_rolling_success_rate(self, tool_name: str) -> float:
        """Get rolling success rate for a tool."""
...
class PrometheusMetrics:
    """Prometheus metrics collector for UCP telemetry."""
    
    def __init__(self) -> None:
        self._router_latency_samples: list[float] = []
        self._tool_invocations: dict[tuple[str, bool], int] = {}
        self._context_shifts: int = 0
    
    def observe_router_latency(self, latency_ms: float) -> None:
        """Record router latency."""
        self._router_latency_samples.append(latency_ms)
    
    def inc_tool_invocation(self, tool_name: str, success: bool) -> None:
        """Increment tool invocation counter."""
        key = (tool_name, success)
        self._tool_invocations[key] = self._tool_invocations.get(key, 0) + 1
    
    def inc_context_shift(self) -> None:
        """Increment context shift counter."""
        self._context_shifts += 1
    
    def get_router_latency_histogram(self) -> str:
        """Generate Prometheus histogram for router latency."""
        if not self._router_latency_samples:
            return ""
        
        samples = sorted(self._router_latency_samples)
        count = len(samples)
        sum_samples = sum(samples)
        
        # Calculate buckets (standard Prometheus buckets: 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000 ms)
        buckets = [5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000]
        lines = []
        
        for bucket in buckets:
            bucket_count = sum(1 for s in samples if s <= bucket)
            lines.append(f'ucp_router_latency_ms_bucket{{le="{bucket}"}} {bucket_count}')
        
        lines.append(f'ucp_router_latency_ms_bucket{{le="+Inf"}} {count}')
        lines.append(f'ucp_router_latency_ms_sum {sum_samples}')
        lines.append(f'ucp_router_latency_ms_count {count}')
        
        return "\n".join(lines)
    
    def get_tool_invocation_counters(self) -> str:
        """Generate Prometheus counters for tool invocations."""
        lines = []
        for (tool_name, success), count in self._tool_invocations.items():
            lines.append(f'ucp_tool_invocations_total{{tool_name="{tool_name}",success="{str(success).lower()}"}} {count}')
        return "\n".join(lines)
    
    def get_context_shift_counter(self) -> str:
        """Generate Prometheus counter for context shifts."""
        return f'ucp_context_shift_detected_total {self._context_shifts}'
    ...
    def get_all_metrics(self) -> str:
        ...
        lines.append("# HELP ucp_router_latency_ms Router decision latency in milliseconds")
        lines.append("# TYPE ucp_router_latency_ms histogram")
        lines.append(self.get_router_latency_histogram())
        lines.append("")
        
        # Tool invocation counters
        lines.append("# HELP ucp_tool_invocations_total Total tool invocations")
        lines.append("# TYPE ucp_tool_invocations_total counter")
        lines.append(self.get_tool_invocation_counters())
        lines.append("")
        
        # Context shift counter
        lines.append("# HELP ucp_context_shift_detected_total Total context shifts detected")
        lines.append("# TYPE ucp_context_shift_detected_total counter")
        lines.append(self.get_context_shift_counter())
        
        return "\n".join(lines)


class JSONLTelemetryExporter:
    """Export telemetry events to JSONL files."""
    ...
    def _get_log_file(self) -> Path:
        """Get the current log file based on date."""
        today = datetime.now(timezone.utc).date()
        
        if today != self._current_date or self._current_file is None:
            self._current_date = today
            filename = f"ucp_telemetry_{today.strftime('%Y-%m-%d')}.jsonl"
            self._current_file = self.logs_dir / filename
        
        return self._current_file
    
    def export_routing_event(self, event: RoutingEvent) -> None:
        """Export a routing event to JSONL."""
        log_file = self._get_log_file()
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(json.dumps(event.to_dict()) + "\n")
    
    def export_tool_call(self, event: ToolCallEvent) -> None:
        """Export a tool call event to JSONL."""
        log_file = self._get_log_file()
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(json.dumps(event.to_dict()) + "\n")
    
    def export_reward(self, reward: RewardSignal) -> None:
        """Export a reward signal to JSONL."""
        log_file = self._get_log_file()
        with open(log_file, "a", encoding="utf-8") as f:
...
def get_jsonl_exporter(logs_dir: str | Path | None = None) -> JSONLTelemetryExporter:
    """Get or create global JSONL exporter."""
    global _jsonl_exporter
    
    if _jsonl_exporter is None:
        dir_path = logs_dir or "./logs"
        _jsonl_exporter = JSONLTelemetryExporter(dir_path)
    
    return _jsonl_exporter
